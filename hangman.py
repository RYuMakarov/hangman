...
def getGuess(alreadyGuessed):
    # Возвращает букву введенную игроком. Эта функция проверяет что игрок ввёл только одну букву и ничего больше.
    ...
    if guess in secretWord:
        ...
        for i in range(len(secretWord)):
            if secretWord[i] not in correctLetters:
                ...
                break
        if foundAllLetters:
            print('ДА! Секретное слово - "' + secretWord + '"! Вы угудали!')
            gameIsDone = True
    else:
        missedLetters = missedLetters + guess

        # Проверяет, превысили ли игрок лимит попыток и проиграл.
